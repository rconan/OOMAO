...
%             out = zeros(size(r));
  ...
  out = ...
  out./(pi*obj.D*obj.D.*(1-obj.obstructionRatio*obj.obstructionRatio)./4);
...
function varargout = footprintProjection(obj,zernModeMax,src)
  ...
  for kSource = 1:nSource
    ...
    for kLayer = 1:obj.atm.nLayer
      ...
      alpha = conjD./obj.D;
      P{kLayer,kSource} = ...
      smallFootprintExpansion(obj.atm.layer(kLayer).zern,delta,alpha);
      varargout{1} = P;
    end
    fprintf('\n')
  end
