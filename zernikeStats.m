...
% See also atmosphere
  ...
  out = phaseStats.spectrum(f,atm).* ...
        fourier(zern_i,f,o).*conj(fourier(zern_i,f,o));
...
% See also atmosphere
  ...
  out = 2*quadgk( ...
  @(nu) zernikeStats.temporalSpectrum(nu,atm,zern).*abs(E(nu)).^2,0,Inf);
  ...
  function out = zernCovCoef(r0,L0,D,i,j,ni,mi,nj,mj)
    ...
    if (mi==mj) && (rem(abs(i-j),2)==0 || ((mi==0) && (mj==0)))
        if L0==Inf
          ...
          for kLayer=1:atm.nLayer
            ...
            out = out + ...
                  atm.layer(kLayer).fractionnalR0* ...
                  (4.*gamma(11./6).^2./pi.^(14./3)).* ...
                  (24.*gamma(6./5)./5).^(5./6).*...
    ...
    % http://www-astro.unice.fr/GSM/Bibliography.html#thesis
      ...
      % pochammerSeries(p,q,a,b,z,tol,nmax)
        ...
        if (p==(q+1) && abs(z)<1) || ...
           (abs(z)==1 && real(sum(a)-sum(b))<0) || p<(q+1)
          ...
          if p==length(a) && q==length(b)
            ...
            error('p and q must be the same length as vectors a and b')
          end
        else
          error('This generalized hypergeometric function does not converge')
        end
      end
    end
...
% See also zernike, atmosphere
  ...
  for cpt = 1:nf
    covValue(cpt) = ...
    zernCovCoef(r0,L0,D,i(cpt),j(cpt),ni(cpt),mi(cpt),nj(cpt),mj(cpt));
  end
  ...
  function out = zernCovCoef(r0,L0,D,i,j,ni,mi,nj,mj)
    if (mi==mj) && (rem(abs(i-j),2)==0 || ((mi==0) && (mj==0)))
      if L0==Inf
        if i==1 && j==1
          ...
          out = (gamma(11./6).^2.*gamma(14./3)./(2.^(8./3).*pi)).* ...
                (24.*gamma(6./5)./5).^(5./6).*...
                (D./r0).^(5./3).* ...
                sqrt((ni+1).*(nj+1)).*(-1).^((ni+nj-mi-mj)./2).*...
                newGamma(-5./6+(ni+nj)./2,...
    ...
    % NEWGAMMA Computes the function defined by Eq.(1.18)
    % in R.J. Sasiela's book:
    ...
    % http://www-astro.unice.fr/GSM/Bibliography.html#thesis
      ...
      b1 = [1+alpha+beta 1+alpha 1+beta];
      a2 = [(1-mu)./2+p 1+p p];
      b2 = [1+(alpha+beta-mu)./2+p, 1+(alpha-beta-mu)./2+p, ...
            1+(beta-alpha-mu)./2+p];
...
% See also zernike, atmosphere, source
  ...
  if nGs>2 % then its a meta-matrix
    if nargin<4 % a correlation meta-matrix
      ...
      for kGs = 1:length(index)
        ...
        buffer{kGs} = ...
        zernikeStats.angularCovariance(zern,atm,[iSrc(iGs),jSrc(jGs)]);
      end
      aiaj(mask) = buffer;
      index = cellfun(@isempty,aiaj);
      aiaj(index) = ...
      cellfun(@transpose,aiaj(triu(~index,1)),'UniformOutput',false);
    ...
    if src(1)==src(2)
      ...
      psdCst = (24.*gamma(6./5)./5).^(5./6).*...
               (gamma(11./6).^2./(2.*pi.^(11./3))).*atm.r0.^(-5./3);
      ...
      aiajFun = @ (znmi,znmj) ...
                quadgk(@(x) ...
                integrand(x,znmi(1),znmi(2),znmi(3),znmj(1),znmj(2),znmj(3)), ...
                0, Inf);
      aiaj = zeros(nMode);
  ...
  function out = integrandNgs(x,zi,ni,mi,zj,nj,mj) %bugged
    ...
    besselsRadialOrder = besselmx('J',ni+1,red1).*besselmx('J',nj+1,red2);
    f = 0.5*x/pi;
    phasePSD = psdCst.*(f.^2 + 1./atm.L0.^2).^(-11./6)./x;
    red = sl*x;
...
function out = tiltsAngularCovariance(zern,atm,src1,varargin)
  ...
  function outSumLayers = sumLayers(f,j,i)
    ...
    outSumLayers = 0;
    for k = 1:atm.nLayer
      srcV = deltaSrc*atm.layer(k).altitude + ...
             lag*atm.layer(k).windSpeed.*exp(1i.*atm.layer(k).windDirection);
      rho = abs(srcV);
      arg = angle(srcV);
      red = 2*pi*f*rho;
...
function out = tiltsTelescopeAngularCovariance(zern1,zern2,atm,src1,varargin)
  ...
  switch tilts
    case 'Z'
      tiltsFilter = @(f) (2*besselj(2,pi.*f.*D1)./(pi.*f.*R1)).* ...
                         (2*besselj(2,pi.*f.*D2)./(pi.*f.*R2));
    case 'G'
      tiltsFilter = @(f) (besselj(1,pi.*f.*D)).^2;
    case'ZG'
      tiltsFilter = @(f) 2*besselj(2,pi.*f.*D).*besselj(1,pi.*f.*D)./(pi.*f.*R);
...
end % tiltsTelescopeAngularCovariance()
...
% out = anisokinetism(zern,atm,src,unit)
  ...
  if ~integral
    ...
    out = 32*pi*cst.*quadgk(@(f) sum( fr0*( f.*(f.^2 + f0Sqrd).^(-11./6).*...
                            (besselj(2,D.*f)./(D.*f)).^2 ).*...
  ...
  if nargin>3
    if isnumeric(unit)
    out = 10^-unit*sqrt(out)*(atm.wavelength/(2*pi));
    elseif ischar(unit)
      if strcmp(unit,'mas')
...
function varargout = residueAngularCovariance( ...
                     sampling,range,modes,atm,srcAC,varargin)
...
% auto- and cross-correlation meta-matrix
  ...
  if isempty(srcCC)
    Cphi_ox = [];
    Cphi_xx = ...
    phaseStats.spatioAngularCovarianceMatrix(sampling,range,atm,srcAC,'mask',p);
  else
    [Cphi_xx,Cphi_ox] = ...
    phaseStats.spatioAngularCovarianceMatrix( ...
      sampling,range,atm,srcAC,srcCC,'mask',p);
  end
  ...
  Cphiai_jpj = cellfun( @(x) zeros(np,zern.nMode), cell(n1,n2) , ...
                        'uniformOutput', false);
  Cphiai_jjp = cellfun( @(x) zeros(np,zern.nMode), cell(n1,n2) , ...
                        'uniformOutput', false);
  ...
  for k1 = 1:n1
    ...
    for k2 = 1:n2;
      ...
      src2Theta = src2(k2).directionVector(1) + ...
                  1i*src2(k2).directionVector(2);
  ...
  Cphizi_jpj  = cellfun( @(x) x*zp', Cphiai_jpj , ...
                         'uniformOutput', false);%  x*zp'+ zp*y'
  Cphizi_jjp  = cellfun( @(y) zp*y', Cphiai_jjp , ...
                         'uniformOutput', false);%  x*zp'+ zp*y'

  switch nargout
    case 1
      varargout{1} = Cphi_xx + cell2mat(Czizj) - cell2mat(Cphizi_jpj) - ...
                     cell2mat(Cphizi_jjp);
    case 2
      if isempty(srcTT)
        u2 = 1:n2;
        varargout{1} = Cphi_xx + ...
                       cell2mat(Czizj(u2,:)) - cell2mat(Cphizi_jpj(u2,:)) - ...
                       cell2mat(Cphizi_jjp(u2,:));
        varargout{2} = cell2mat(Cphi_ox) - cell2mat(Cphizi_jpj(n1,:));
      else
        nTT = length(srcTT);
        u2 = nTT+1:n2;
        C_xjxjp = Cphi_xx + cell2mat(Czizj(u2,u2)) - ...
                  cell2mat(Cphizi_jpj(u2,u2)) - cell2mat(Cphizi_jjp(u2,u2));
        u = 1:nTT;
        C_aijaipjp = cell2mat(aiaj(u,u));
        C_aijXjp_row   = cell2mat(Cphiai_jjp(u,u2)')' - ...
        cell2mat( cellfun( @(x) x*zp', aiaj(u,u2) , 'uniformOutput', false) );
...
end % residueAngularCovariance()

function varargout = residueAngularCovariance_ll( ...
                     sampling,range,modes,atm,srcAC,varargin)
%% RESIDUEANGULARCOVARIANCE Residual phase spatio-angular covariance meta matrix
%
% [S,C] = residueAngularCovariance(sampling,range,modes,atm,src1)
...
% auto- and cross-correlation meta-matrix
  ...
  switch nargout
    case 1
        varargout{1} = Cphi_xx + cell2mat(Czizj) - ...
                       cell2mat(Cphizi_jpj) - cell2mat(Cphizi_jjp);
    case 2
      if isempty(srcTT)
        u2 = 1:n2;
        varargout{1} = Cphi_xx + cell2mat(Czizj(u2,:)) - ...
                       cell2mat(Cphizi_jpj(u2,:)) - cell2mat(Cphizi_jjp(u2,:));
        varargout{2} = cell2mat(Cphi_ox) - cell2mat(Cphizi_jpj(n1,:));
      else
        nTT = length(srcTT);
        u2 = nTT+1:n2;
...
end % residueAngularCovariance_ll()

function varargout = residueVarianceMap(N,r,o,atm,tel)
%% RESIDUEVARIANCEMAP

  persistent aiaj zMode zN zM R L0 L0r0Ratio L0RRatio c1 c2 red1 atmVar
  if isempty(aiaj)
    ...
    red1   = 2*pi*R/L0;
    r      = r/R;
    atmVar = c1.*(gamma(11./6).*gamma(5./6)./(2.*pi.^(8./3))).*L0r0Ratio;
...
end % residueVarianceMap()

function varargout = residueStructureFunction(N,r1,o1,r2,o2,atm,tel)
%% RESIDUESTRUCTUREFUNCTION

  persistent aiaj zMode zN zM R L0 L0r0Ratio L0RRatio c1 c2 cst red1 atmVar
  if isempty(aiaj)
    ...
    cst           = c1.*(gamma(11./6)./(2.^(5./6).*pi.^(8./3))).*L0r0Ratio;
  ...
  for zi = 1:N
    ...
    for zj = 1:N
      ...
      if (mi==mj) && (rem(abs(zi-zj),2)==0 || ((mi==0) && (mj==0)))
        ...
        zernCov = zernCov + aiaj(zi,zj).*(zerni1-zerni2).*(zernj1-zernj2);
      end
    end
  end
  ...
  zernPhaseCovFun = ...
  @(r1_,o1_,r2_,o2_) quadgk(@(x) CphiAi(x,r1_,o1_,r2_,o2_), 0, Inf);
  parfor kR = 1:nR
...
%% RESIDUEOTF
  ...
  function out1 = residueOtfInt(oo,rr)
    ...
    o2 = angle(z2);
    out1 = rr.* ...
    exp(-0.5*zernikeStats.residueStructureFunction(N,r1,o1,r2,o2,atm,tel));
  end
