...
end % sparseInverseCovarianceMatrix()
...
%% SPECTRUM Phase power spectrum density for von Karman turbulence with
%% outer scale L0 and zero inner scale.
%
...
%% OTF Phase optical transfer function
%
% out = phaseStats.otf(rho) compute the phase optical transfer function
...
% See also atmosphere
  ...
  if max(nRho,mRho)>blockSize % Memory gentle
    ...
    out = cell2mat( ...
          cellfun(@(x) phaseStats.covariance(x,atm), rho, ...
          'UniformOutput', false) );
...
% See also atmosphere
  ...
  c  = ...
  phaseStats.covariance( abs( bsxfun(@minus , z2(1:nz:nz^2), z1(1:nz).') ), ...
                         atm );
  ...
  r = ...
  phaseStats.covariance( abs( bsxfun(@minus , z1(1:nz:nz^2), z2(1:nz).') ), ...
                         atm );
  r   = mat2cell( r , nz , ones(mz,1) );
  % First blocks column
...
end % covarianceToeplitzMatrix()

function varargout = ...
         spatioAngularCovarianceMatrix(sampling,range,atm,srcAC,varargin)
...
end % spatioAngularCovarianceMatrix()
...
function C = crossCorrelationTT(srcTT,x,y,mask,...
  ...
  C = cellfun( @(x) zeros(sum(mask(:))) , cell(nSs,nGs) , ...
               'UniformOutput' , false );
  x = x(mask);
  y = y(mask);
  f02 = 1./L0.^2;
...
function C = crossCorrelation(srcCC,xAC,yAC,xCC,yCC,...
    ...
    C = cellfun( @(x) zeros(length(xCC),length(xAC)), cell(nSs,nGs), ...
                'UniformOutput' , false );
...
function S = autoCorrelation(x,y,mask,...
  ...
  kGs(1) = 1;
  kGs(kGs==0) = [];
  S = cellfun( @(x) zeros(sum(mask(:))) , cell(1,length(kGs)) , ...
               'UniformOutput' , false );
  ...
  parfor k=1:length(kGs)
    ...
    for kLayer=1:nLayer
        ...
        % First block rows
        rr   = cellfun( @(x,y) myToeplitz(x,y) , c , r , ...
                        'UniformOutput' , false);
        ...
        r   = mat2cell( fr0(kLayer)*r , nz , ones(mz,1) );
        % First blocks column
        cc   = cellfun( @(x,y) myToeplitz(x,y) , c , r , &
                        'UniformOutput' , false);
...
% See also zernike, atmosphere
  ...
  function out = zernCovCoef(r0,L0,D,i,j,ni,mi,nj,mj)
    ...
    % http://www-astro.unice.fr/GSM/Bibliography.html#thesis
      ...
      a2 = [(1-mu)./2+p 1+p p];
      b2 = [1+(alpha+beta-mu)./2+p, ...
            1+(alpha-beta-mu)./2+p, 1+(beta-alpha-mu)./2+p];
...
% See also zernike, atmosphere
  ...
  covValue = zeros(1,nf);
  for cpt = 1:nf
    covValue(cpt) = zernCovCoef(r0,L0,D,i(cpt),j(cpt), ...
                                ni(cpt),mi(cpt),nj(cpt),mj(cpt));
  end
        ...
        if (p==(q+1) && abs(z)<1) || ...
          ...
          if p==length(a) && q==length(b)
            ...
            if ~isempty(indnz)
              ...
              error('The generalized hypergeometric function does not converge')
            end
        end
      end
...
% See also zernike, atmosphere, source
    ...
    if nGs>2 % then its a meta-matrix
      ...
      if nargin<4 % a correlation meta-matrix
        ...
        for kGs = 1:length(index)
          ...
          buffer{kGs} = ...
          phaseStats.zernikeAngularCovariance(zern,atm,[iSrc(iGs),jSrc(jGs)]);
        end
        aiaj(mask) = buffer;
      ...
      parfor kGs = 1:nGs*mGs
        ...
        aiaj{kGs} = phaseStats.zernikeAngularCovariance(zern,atm, ...
                                                       [iSrc(iGs),jSrc(jGs)]);
      end
    end
    else
    ...
    function out = integrand(x,zi,ni,mi,zj,nj,mj)
      ...
      for kLayer=1:atm.nLayer
        ...
        besselsRadialOrder = besselmx('J',ni+1,red1).* ...
                             besselmx('J',nj+1,red2);
